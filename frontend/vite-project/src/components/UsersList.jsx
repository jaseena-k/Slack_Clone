...
export const UsersList = ({activeChannel}) => {
    ...
    const {data:users =[],
        ...
        queryKey:['user-list',client?.user?.id],
        queryFn:fetchUsers,
        enabled:!!client?.user,
        staleTime:1000*60*5
    ...
    const startDirectMessage = async (targetUser)=>{
        if(!targetUser || !client?.user?.id) return
    ...
    if(isLoading) return <div className='team-channel-list-message'>Loading users...</div>
    if(isError) return <div className='team-channel-list-message'>Failed to load  users</div>
    if(!users.length) return <div className='team-channel-list-message'>No other Users Found</div>
